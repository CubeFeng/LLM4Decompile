...
try:
    ...
    HAS_RICH = False
    logger.warning("未安装rich库，将使用基本控制台显示")
...
class ResourceMonitor:
    ...
    def _check_alerts(self, metrics: ResourceMetrics):
        ...
        if (metrics.gpu_memory_used_mb and metrics.gpu_memory_total_mb and
                (metrics.gpu_memory_used_mb / metrics.gpu_memory_total_mb * 100) > self.alert_thresholds[
                    'gpu_memory_percent']):
    ...
    def _clear_monitor_display(self):
        """清理监控显示区域"""
        if HAS_RICH and self._rich_live:
            # 停止rich live显示
    ...
    def _display_realtime_status(self):
        ...
        if HAS_RICH and self._rich_console:
            # 使用rich进行分区域显示
            if not self._rich_live and self._realtime_enabled:
                self.logger.debug("初始化Rich实时显示")
                # 创建一个空的占位符
    ...
    def print_current_status(self):
        ...
        if metrics.gpu_memory_used_mb is not None and metrics.gpu_memory_total_mb is not None:
            ...
            print(
                f"GPU显存: {metrics.gpu_memory_used_mb:.1f}/{metrics.gpu_memory_total_mb:.1f} MB ({memory_percent:.1f}%)")
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        health_status = {
            ...
            'gpu_available': self.gpu_collector.initialized if hasattr(self,
                                                                       'gpu_collector') and self.gpu_collector else False,
