#!/usr/bin/env bash
# Benchmark serial decompile.py vs parallel DecompileParallel.java (consumer-stream v2).
set -euo pipefail
...
PY
  ...
  echo "${label},seconds=${elapsed},functions=${markers},mode=${mode},output=${output_path},log=${stdout_log}"
}
...
run_case "serial" "decompile.py"
run_case "parallel" "DecompileParallel.java" "${CHUNK_THRESHOLD}" "${GHIDRA_MAX_CPU}" "${SINGLE_QUEUE_LIMIT}"
