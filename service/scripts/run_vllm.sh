...
if [[ ! -x "${PYTHON_BIN}" ]]; then
  cat >&2 <<EOF
ERROR: VLLM_PYTHON is not executable:
  ${PYTHON_BIN}

Set VLLM_PYTHON in ${ENV_FILE} to the absolute Python path for the environment that has vLLM installed.
EOF
  exit 1
fi
...
echo "Preparing vLLM OpenAI-compatible server"
